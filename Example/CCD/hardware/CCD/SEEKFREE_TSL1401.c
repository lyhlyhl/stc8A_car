...
//-------------------------------------------------------------------------------------------------------------------
void ccd_send_data(UARTN_enum uart_n ,u16 *dat)
{
   u8 i = 0;
   ...
   uart_putchar(uart_n, 0x00);
		...
		for(i=0; i<128; i++)        
	{
		uart_putchar(uart_n, (u8)dat[i]>>8);   //发送高8位
		uart_putchar(uart_n, (u8)dat[i]&0XFF); //发送高低8位	               
	}
//   	uart_putchar(uart_n, 0x0d);
//		uart_putchar(uart_n, 0x0a);
//	for(i=0; i<128; i++)        
//	{
//		sprintf(str, "%d",ccd_data[i]);
//		uart_putstr(uart_n,str); 
//		uart_putchar(uart_n, 0x20);
//	//	uart_putchar(uart_n, (u8)dat[i]>>8);   //发送高8位
//	//	uart_putchar(uart_n, (u8)dat[i]&0XFF); //发送高低8位	               
//	}
//		uart_putchar(uart_n, 0x0d);
